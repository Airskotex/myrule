...
download_runfile() {
    ...
    if [ -f "./${filename}" ]; then
        ...
        mv -f "./${filename}" "./${corrupt_file}" || true
...
EOF
    ...
    read -p "$(echo -e "${BOLD}是否立即重启?${NC} ${YELLOW}(y/N): ${NC}")" REBOOT_NOW
...
if [ -f "$STATE_FILE" ]; then
    ...
    while true; do
        ...
        for driver_version in "${DRIVER_VERSIONS[@]}"; do
            if [[ "$driver_version" == "$RECOMMENDED_DRIVER_VERSION" ]]; then
                DRIVER_OPTIONS+=("${driver_version} (推荐, CUDA自带)")
            else
        ...
        for ((i = 0; i < ${#DRIVER_OPTIONS[@]}; i++)); do
            OPTION_TEXT="${DRIVER_OPTIONS[$i]}"
            if [[ "$OPTION_TEXT" == *"推荐"* ]]; then
                printf "  ${CYAN}%2d)${NC} ${BOLD}${GREEN}%s${NC}\n" "$((i + 1))" "$OPTION_TEXT"
            elif [[ "$OPTION_TEXT" == "返回上级菜单 (重新选择 CUDA)" ]]; then
                printf "  ${CYAN}%2d)${NC} ${BLUE}%s${NC}\n" "$((i + 1))" "$OPTION_TEXT"
            elif [[ "$OPTION_TEXT" == "不安装 NVIDIA Driver (仅安装 CUDA Toolkit)" ]]; then
        ...
        while true; do
            ...
            if ! [[ "$DRIVER_INPUT" =~ ^[0-9]+$ ]] || (( DRIVER_INPUT < 1 || DRIVER_INPUT > ${#DRIVER_OPTIONS[@]} )); then
                log_error "无效选项，请重新输入。"
                continue
            fi

            DRIVER_SELECT="${DRIVER_OPTIONS[$((DRIVER_INPUT - 1))]}"
            if [[ "$DRIVER_SELECT" == "返回上级菜单 (重新选择 CUDA)" ]]; then
                ...
                if [[ "$SELECTED_DRIVER_VERSION" == "$RECOMMENDED_DRIVER_VERSION" ]]; then
                    SELECTED_DRIVER_SOURCE="cuda"
                    SELECTED_DRIVER_FILE=""
                    SELECTED_DRIVER_URL=""
                else
...
echo -e "${BOLD}${BLUE}=====================================================${NC}"
echo -e "验证命令: ${GREEN}source ~/.bashrc${NC} 之后运行 ${GREEN}nvcc -V${NC}"
