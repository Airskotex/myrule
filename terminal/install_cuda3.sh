...
install_aria2_if_missing() {
    ...
    if command -v apt-get &> /dev/null; then
        if sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y aria2 || \
            { sudo apt-get update && sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y aria2; }; then
            :
        fi
        ...
        sudo zypper --non-interactive install aria2 || true
...
configure_cuda_environment() {
    ...
    cuda_env_block | sudo tee "$profile_file" >/dev/null
    sudo chmod 644 "$profile_file"

    append_cuda_env_to_rc "$HOME/.bashrc"
    if [[ "${SHELL:-}" == */zsh || -f "$HOME/.zshrc" ]]; then
        append_cuda_env_to_rc "$HOME/.zshrc"
    fi
}
...
download_file() {
    ...
    if command -v aria2c &> /dev/null; then
        ...
        log_warn "aria2c 下载失败，回退到 wget 断点续传。"
    else
...
disable_nouveau_and_reboot() {
    ...
    # 1. 写入黑名单
    sudo tee "$NVIDIA_BLACKLIST_FILE" >/dev/null <<EOF
blacklist nouveau
...
EOF
    ...
    if [[ "$ID" == "ubuntu" || "$ID" == "debian" ]]; then
        ...
        if ! sudo update-initramfs -u; then
            log_error "无法更新 initramfs，请手动执行后再重新运行脚本。"
            exit 1
        fi
    fi
...
if [[ "$CONFIRM_DRIVER" == "yes" ]]; then
    ...
    if [[ "$SELECTED_DRIVER_SOURCE" != "cuda" ]]; then
        ...
        if ! sudo "$(runfile_path "$SELECTED_DRIVER_FILE")" --silent --no-questions; then
...
if [[ "$INSTALL_MODE" == "driver_only" ]]; then
    ...
    echo -e "${BOLD}${BLUE}=====================================================${NC}"
    echo -e "手动复验: ${GREEN}source /etc/profile.d/cuda.sh 2>/dev/null || true; nvcc -V; nvidia-smi${NC}"
    exit 0
fi
...
if ! sudo "$(runfile_path "$FILENAME")" "${CUDA_INSTALL_ARGS[@]}"; then
