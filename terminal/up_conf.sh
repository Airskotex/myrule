...
check_network() {
    log_info "检查网络连通性..."
    if ! ping -c 1 -W 5 mirrors.aliyun.com >/dev/null 2>&1;
...
if [ "$(id -u)" -ne 0 ]; then
   log_error "此脚本需要以 root 权限运行。请使用 'sudo ./leon_upconf.sh'。"
   exit 1
fi
...
install_desktop() {
    ...
    apt install -y xorgxrdp xserver-xorg-core xserver-xorg-video-intel || log_warn "部分XRDP依赖组件安装失败，但可能不影响基本功能"
}
