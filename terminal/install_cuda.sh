...
NVIDIA_BLACKLIST_FILE="${NVIDIA_BLACKLIST_FILE:-/etc/modprobe.d/blacklist-nouveau.conf}"
SYMLINK_PATH="${CUDA_SYMLINK_PATH:-/usr/local/cuda}"
...
NVIDIA_PROBE_TIMEOUT_SECONDS="${NVIDIA_PROBE_TIMEOUT_SECONDS:-30}"
...
AUTO_INSTALL_DEPENDENCIES="${AUTO_INSTALL_DEPENDENCIES:-1}" # 设为 0 时仅报告缺少依赖，不自动安装
CUDA_INSTALL_DRY_RUN="${CUDA_INSTALL_DRY_RUN:-0}" # 设为 1 时仅执行发现和预检，不修改系统
CUDA_INSTALL_MODE="${CUDA_INSTALL_MODE:-}" # cuda 或 driver_only；留空时使用交互菜单
CUDA_DRIVER_PACKAGE="${CUDA_DRIVER_PACKAGE:-}" # Ubuntu 26.04 非交互选择的 Driver 包
OS_RELEASE_FILE="${OS_RELEASE_FILE:-/etc/os-release}"
KERNEL_RELEASE_OVERRIDE="${KERNEL_RELEASE_OVERRIDE:-}"
NVIDIA_PROC_VERSION_FILE="${NVIDIA_PROC_VERSION_FILE:-/proc/driver/nvidia/version}"
...
UBUNTU_BASE_PACKAGES=(ca-certificates wget gzip coreutils)
UBUNTU_DRIVER_PACKAGES=(ubuntu-drivers-common pciutils kmod mokutil dkms build-essential)
APT_DRIVER_PACKAGES=()
APT_DRIVER_VERSIONS=()
APT_DRIVER_CANDIDATES=()
APT_DRIVER_LABELS=()
APT_RECOMMENDED_PACKAGE=""
...
collect_missing_packages() {
    ...
    result_ref=()
    for package in "$@"; do
        if ! valid_apt_package_name "$package"; then
            log_error "不合法的 APT 包名: ${package}"
            return 1
        fi
        package_is_installed "$package" || result_ref+=("$package")
    done
}

ensure_apt_metadata() {
    if [[ "$APT_METADATA_REFRESHED" == "1" ]]; then
        return 0
    fi

    if ! run_privileged apt-get update; then
        log_error "apt-get update 失败，无法安装缺少的依赖。"
        return 1
    fi
...
ensure_ubuntu_packages() {
    ...
    command -v apt-get &> /dev/null || {
        log_error "Ubuntu 26.04 缺少 apt-get，无法自举 ${label}。"
        return 1
    }
    command -v dpkg-query &> /dev/null || {
        log_error "Ubuntu 26.04 缺少 dpkg-query，无法检查 ${label}。"
        return 1
    }

    collect_missing_packages missing "$@" || return 1
    if (( ${#missing[@]} == 0 )); then
        return 0
    fi

    log_warn "缺少 ${label}: ${missing[*]}"
    if [[ "$AUTO_INSTALL_DEPENDENCIES" != "1" ]]; then
        print_hint "AUTO_INSTALL_DEPENDENCIES=${AUTO_INSTALL_DEPENDENCIES}，未自动安装。"
        return 1
    fi

    if [[ "$CUDA_INSTALL_DRY_RUN" == "1" ]]; then
        print_hint "DRY-RUN 将安装: apt-get install -y -- ${missing[*]}"
        return 0
    fi

    ensure_apt_metadata || return 1
    if ! run_privileged env DEBIAN_FRONTEND=noninteractive apt-get install -y -- "${missing[@]}"; then
        log_error "安装 ${label} 失败: ${missing[*]}"
        return 1
    fi

    for package in "${missing[@]}"; do
        if ! package_is_installed "$package"; then
            log_error "APT 返回成功，但依赖仍未安装: ${package}"
            return 1
        fi
    done
}
...
ensure_base_dependencies() {
    local backend=${DRIVER_BACKEND:-}

    [[ -n "$backend" ]] || backend=$(detect_driver_backend) || return 1
    if [[ "$backend" == "apt" ]]; then
        ensure_ubuntu_packages "CUDA 下载依赖" "${UBUNTU_BASE_PACKAGES[@]}"
        return $?
    fi
...
ensure_driver_dependencies() {
    local kernel_release
    local headers_package
    local -a packages=()

    [[ "$DRIVER_BACKEND" == "apt" ]] || return 0

    kernel_release=$(current_kernel_release) || return 1
    headers_package="linux-headers-${kernel_release}"
    packages=("${UBUNTU_DRIVER_PACKAGES[@]}" "$headers_package")
    ensure_ubuntu_packages "NVIDIA Driver 依赖" "${packages[@]}" || return 1

    if [[ "$CUDA_INSTALL_DRY_RUN" != "1" ]] && ! package_is_installed "$headers_package"; then
        log_error "当前内核头文件仍不可用: ${headers_package}"
        return 1
    fi
}
...
install_aria2_if_missing() {
    ...
    if [[ "$CUDA_INSTALL_DRY_RUN" == "1" ]]; then
        print_hint "DRY-RUN 将尝试安装可选包 aria2；当前继续使用 wget。"
        return 0
    fi
...
read_os_release_values() {
    local id_var=$1
    local version_var=$2
    local parsed_id=""
    local parsed_version_id=""
    ...
    while IFS='=' read -r key value; do
        ...
        case "$key" in
            ID) parsed_id=$value ;;
            VERSION_ID) parsed_version_id=$value ;;
        esac
    done < "$OS_RELEASE_FILE"

    printf -v "$id_var" '%s' "$parsed_id"
    printf -v "$version_var" '%s' "$parsed_version_id"
}
...
load_os_release() {
    ...
    if [[ ! -r "$OS_RELEASE_FILE" ]]; then
        log_error "无法确定操作系统版本: ${OS_RELEASE_FILE}"
        return 1
    fi
    ...
    while IFS='=' read -r key value; do
        case "$key" in
            ID|ID_LIKE|VERSION_ID|PRETTY_NAME|NAME|VERSION_CODENAME|UBUNTU_CODENAME)
                value="${value%\"}"
                value="${value#\"}"
                value="${value%\'}"
                value="${value#\'}"
    ...
    [[ -n "$ID" ]] || {
        log_error "os-release 缺少 ID: ${OS_RELEASE_FILE}"
        return 1
    }
}

detect_driver_backend() {
    local os_id=""
    local os_version_id=""

    read_os_release_values os_id os_version_id || return 1
...
initialize_installer() {
    ...
    init_privilege
    load_os_release
    DRIVER_BACKEND=$(detect_driver_backend) || {
        log_error "无法选择 NVIDIA Driver 安装后端。"
        return 1
    }
    print_kv "Driver 后端" "$DRIVER_BACKEND"

    ensure_base_dependencies || return 1
...
# 函数：判断字符串是否为纯数字点分版本号(如 580、595.71.05)。
# 驱动版本多来自 nvidia-smi/apt 等外部命令输出，失败时它们会把错误信息当正常结果吐出来，
# 必须先验形状再进数值比较，否则 $((10#...)) 会直接抛算术错误。
is_version_string() {
    [[ "$1" =~ ^[0-9]+(\.[0-9]+)*$ ]]
}
...
version_ge() {
    ...
    # 非法版本号一律判为“不可比较”，返回 1 而不是让算术展开报错中断调用方。
    is_version_string "$candidate" || return 1
    is_version_string "$minimum" || return 1
...
prepare_driver_only_driver_versions() {
    ...
    if [[ "$DRIVER_BACKEND" == "apt" ]]; then
        prepare_apt_driver_only_packages
        return $?
    fi
...
prepare_apt_driver_only_packages() {
    ...
    active_cuda=$(detect_active_cuda_version || true)
    if [[ -n "$active_cuda" ]]; then
        DRIVER_ONLY_INSTALLED_CUDA="$active_cuda"
        release_notes_cuda=$(resolve_cuda_archive_version "$active_cuda")
        [[ -n "$release_notes_cuda" ]] || release_notes_cuda="$active_cuda"
        minimum_driver=$(get_minimum_driver_version "$release_notes_cuda")
        recommended_driver=$(get_recommended_driver_version "$release_notes_cuda")
        if [[ -z "$minimum_driver" && -n "$recommended_driver" ]]; then
            minimum_driver="$recommended_driver"
            log_warn "未获取到最低 Driver 版本，使用 CUDA ${active_cuda} 配套版本 ${recommended_driver} 作为下限。"
        fi
    fi
    ...
    DRIVER_ONLY_MINIMUM_DRIVER="$required_driver"
    if [[ -n "$active_cuda" ]]; then
        print_section "按当前 CUDA 过滤 Ubuntu Driver 包"
        print_kv "当前 CUDA" "$active_cuda"
    fi
    [[ -z "$required_driver" ]] || print_kv "最低兼容驱动" "$required_driver"

    discover_apt_driver_packages "$required_driver"
}
...
valid_apt_driver_package() {
    [[ "$1" =~ ^nvidia-driver-[0-9]+(-open|-server|-server-open)?$ ]]
}

apt_candidate_version() {
    local package=$1

    valid_apt_driver_package "$package" || return 1
    env LC_ALL=C apt-cache policy "$package" 2>/dev/null | \
        awk '$1 == "Candidate:" && $2 != "(none)" { print $2; exit }'
}

normalize_apt_driver_version() {
    local candidate=$1
...
discover_apt_driver_packages() {
    local minimum_driver=${1:-}
    local devices_output
    local line package candidate version priority recommended
    local records=""
    ...
    command -v ubuntu-drivers &> /dev/null || {
        log_error "未找到 ubuntu-drivers，无法发现 Ubuntu 26.04 官方 Driver 包。"
        return 1
    }
    command -v apt-cache &> /dev/null || {
        log_error "未找到 apt-cache，无法确认 Ubuntu 26.04 Driver candidate。"
        return 1
    }

    devices_output=$(ubuntu-drivers devices 2>/dev/null) || {
        log_error "ubuntu-drivers devices 执行失败。"
        return 1
    }

    while IFS= read -r line; do
        [[ "$line" =~ driver[[:space:]]*:[[:space:]]*(nvidia-driver-[0-9]+(-open|-server|-server-open)?) ]] || continue
        package=${BASH_REMATCH[1]}
        valid_apt_driver_package "$package" || continue

        candidate=$(apt_candidate_version "$package" || true)
        [[ -n "$candidate" ]] || continue
        version=$(normalize_apt_driver_version "$candidate" || true)
        [[ -n "$version" ]] || continue
        if [[ -n "$minimum_driver" ]] && ! version_ge "$version" "$minimum_driver"; then
            continue
        fi

        recommended=0
        [[ "$line" == *" recommended"* ]] && recommended=1
        priority=1
        (( recommended == 1 )) && priority=0
        records+="${priority}"$'\t'"${version}"$'\t'"${package}"$'\t'"${candidate}"$'\n'
    done <<< "$devices_output"

    while IFS=$'\t' read -r priority version package candidate; do
        [[ -n "$package" ]] || continue
        APT_DRIVER_PACKAGES+=("$package")
        APT_DRIVER_VERSIONS+=("$version")
        APT_DRIVER_CANDIDATES+=("$candidate")
        APT_DRIVER_LABELS+=("${version} (${package})")
        if [[ "$priority" == "0" && -z "$APT_RECOMMENDED_PACKAGE" ]]; then
            APT_RECOMMENDED_PACKAGE="$package"
        fi
    done < <(printf '%s' "$records" | \
        awk -F '\t' '!seen[$3]++' | \
        LC_ALL=C sort -t $'\t' -k1,1n -k2,2rV)

    if (( ${#APT_DRIVER_PACKAGES[@]} == 0 )); then
        if [[ -n "$minimum_driver" ]]; then
            log_error "没有找到满足最低版本 ${minimum_driver} 的 Ubuntu Driver 包。"
        else
            log_error "没有找到可安装的 Ubuntu Driver 包。"
        fi
        return 1
    fi

    if [[ -z "$APT_RECOMMENDED_PACKAGE" ]]; then
        APT_RECOMMENDED_PACKAGE="${APT_DRIVER_PACKAGES[0]}"
    fi
}

apt_driver_package_is_listed() {
    local candidate=$1
    local package

    for package in "${APT_DRIVER_PACKAGES[@]}"; do
        [[ "$candidate" == "$package" ]] && return 0
    done
    return 1
}

select_apt_driver_package() {
    local package=$1
    local i

    valid_apt_driver_package "$package" || {
        log_error "不合法的 Ubuntu Driver 包名: ${package}"
        return 1
    }

    for ((i = 0; i < ${#APT_DRIVER_PACKAGES[@]}; i++)); do
        if [[ "${APT_DRIVER_PACKAGES[$i]}" == "$package" ]]; then
            CONFIRM_DRIVER="yes"
            SELECTED_DRIVER_PACKAGE="$package"
            SELECTED_DRIVER_VERSION="${APT_DRIVER_VERSIONS[$i]}"
            SELECTED_DRIVER_SOURCE="apt"
    ...
    log_error "当前 GPU/软件源不可安装指定包: ${package}"
    return 1
}

print_apt_driver_packages() {
    local start_option=$1
    local i option label

    for ((i = 0; i < ${#APT_DRIVER_PACKAGES[@]}; i++)); do
        option=$((start_option + i))
        label="${APT_DRIVER_LABELS[$i]}"
        if [[ "${APT_DRIVER_PACKAGES[$i]}" == "$APT_RECOMMENDED_PACKAGE" ]]; then
            label+=" (Ubuntu 推荐)"
        fi
        printf "  ${CYAN}%2d)${NC} %s\n" "$option" "$label"
    done
...
print_apt_driver_selection_result() {
    print_kv "已选择 NVIDIA Driver" "$SELECTED_DRIVER_VERSION"
    print_kv "Ubuntu Driver 包" "$SELECTED_DRIVER_PACKAGE"
    print_kv "驱动来源" "Ubuntu/NVIDIA 官方 APT 软件源"
}

select_apt_driver_for_cuda() {
    local input=""
    local index

    discover_apt_driver_packages "$MINIMUM_DRIVER_VERSION" || return 1

    if [[ -n "$CUDA_DRIVER_PACKAGE" ]]; then
        select_apt_driver_package "$CUDA_DRIVER_PACKAGE" || return 1
        print_apt_driver_selection_result
        return 0
    fi

    while true; do
        print_section "选择 Ubuntu NVIDIA Driver 包"
        [[ -n "$MINIMUM_DRIVER_VERSION" ]] && print_kv "兼容范围" "Driver >= ${MINIMUM_DRIVER_VERSION}"
        print_hint "Ubuntu 26.04 使用官方 APT/Open Kernel Modules，不调用 Driver runfile。"
        printf "  ${CYAN}%2d)${NC} ${BLUE}%s${NC}\n" 1 "返回上级菜单 (重新选择 CUDA)"
        printf "  ${CYAN}%2d)${NC} ${YELLOW}%s${NC}\n" 2 "不安装 NVIDIA Driver (仅安装 CUDA Toolkit)"
        print_apt_driver_packages 3

        if ! read -r -t "$DRIVER_SELECT_TIMEOUT" -p "$(echo -e "${BOLD}${CYAN}请输入选项编号:${NC} ")" input; then
            echo
            log_warn "超过 ${DRIVER_SELECT_TIMEOUT} 秒未选择，使用 Ubuntu 推荐包 ${APT_RECOMMENDED_PACKAGE}。"
            select_apt_driver_package "$APT_RECOMMENDED_PACKAGE" || return 1
            print_apt_driver_selection_result
            return 0
        fi
        ...
        if [[ "$input" == "1" ]]; then
            ...
            return 0
             ...
             (( input >= 3 && input < 3 + ${#APT_DRIVER_PACKAGES[@]} )); then
            index=$((input - 3))
            select_apt_driver_package "${APT_DRIVER_PACKAGES[$index]}" || return 1
            print_apt_driver_selection_result
            return 0
        fi
...
choose_install_mode() {
    if [[ -n "$CUDA_INSTALL_MODE" ]]; then
        case "$CUDA_INSTALL_MODE" in
            cuda|driver_only)
                INSTALL_MODE="$CUDA_INSTALL_MODE"
                print_kv "非交互安装模式" "$INSTALL_MODE"
                return 0
                ;;
            *)
                log_error "CUDA_INSTALL_MODE 取值无效: ${CUDA_INSTALL_MODE}（可选 cuda 或 driver_only）。"
                return 1
                ;;
        esac
    fi
...
select_apt_driver_only_flow() {
    local input=""
    local index

    print_section "获取 Ubuntu NVIDIA Driver 包列表"
    prepare_apt_driver_only_packages || return 1
    ...
    while true; do
        print_section "选择 Ubuntu NVIDIA Driver 包"
        print_hint "当前模式: 仅安装 NVIDIA Driver，不安装 CUDA Toolkit。"
        [[ -z "$DRIVER_ONLY_INSTALLED_CUDA" ]] || print_kv "当前 CUDA" "$DRIVER_ONLY_INSTALLED_CUDA"
        [[ -z "$DRIVER_ONLY_MINIMUM_DRIVER" ]] || print_kv "兼容范围" "Driver >= ${DRIVER_ONLY_MINIMUM_DRIVER}"
        printf "  ${CYAN}%2d)${NC} ${BLUE}%s${NC}\n" 1 "返回安装模式选择"
        print_apt_driver_packages 2

        if ! read -r -p "$(echo -e "${BOLD}${CYAN}请输入选项编号:${NC} ")" input; then
            echo
            log_error "未读取到输入（EOF），已退出。"
            return 1
        fi
        ...
        if [[ "$input" == "1" ]]; then
            return 1
             ...
             (( input >= 2 && input < 2 + ${#APT_DRIVER_PACKAGES[@]} )); then
            index=$((input - 2))
            select_apt_driver_package "${APT_DRIVER_PACKAGES[$index]}" || return 1
            print_apt_driver_selection_result
            CUDA_VERSION=""
...
save_install_state() {
    ...
    (
        umask 077
        {
            ...
            write_state_field "SAVED_DRIVER_BACKEND" "${DRIVER_BACKEND:-runfile}"
            write_state_field "SAVED_DRIVER_PACKAGE" "${SELECTED_DRIVER_PACKAGE:-}"
        } > "$temp_state"
    )
...
load_install_state() {
    ...
    SAVED_DRIVER_BACKEND=""
    SAVED_DRIVER_PACKAGE=""
    ...
    while IFS='=' read -r key value || [[ -n "$key" ]]; do
        ...
        case "$key" in
            ...
            SAVED_DRIVER_BACKEND) SAVED_DRIVER_BACKEND="$value" ;;
            SAVED_DRIVER_PACKAGE) SAVED_DRIVER_PACKAGE="$value" ;;
...
validate_driver_source() {
    ...
    [[ -z "$source" || "$source" == "cuda" || "$source" == "archive" || "$source" == "apt" ]]
}

validate_driver_backend() {
    [[ "$1" == "apt" || "$1" == "runfile" ]]
}
...
detect_loaded_driver_version() {
    ...
    if [[ -r "$NVIDIA_PROC_VERSION_FILE" ]]; then
        version=$(grep -m1 -oE '[0-9]+\.[0-9]+(\.[0-9]+)?' "$NVIDIA_PROC_VERSION_FILE" 2>/dev/null | head -1)
    fi
...
# 函数：执行 nvidia-smi 查询，仅在驱动真正可用时输出结果。
# 驱动未加载时 nvidia-smi 退出码非 0，且把 "NVIDIA-SMI has failed because it couldn't
# communicate with the NVIDIA driver" 打到 stdout，因此 2>/dev/null 拦不住，
# 必须同时校验退出码和内容，否则这段横幅会被当成 GPU 名或驱动版本继续往下传。
nvidia_smi_query() {
    local output=""

    command -v nvidia-smi &> /dev/null || return 1

    output=$(timeout --signal=TERM --kill-after=5s "${NVIDIA_PROBE_TIMEOUT_SECONDS:-30}" \
        nvidia-smi "$@" 2>/dev/null) || return 1
    [[ -n "$output" ]] || return 1
    grep -qiE "NVIDIA-SMI has failed|couldn't communicate|No devices were found" <<< "$output" && return 1
...
detect_gpu_driver_floor() {
    ...
    smi_names=$(nvidia_smi_query --query-gpu=name --format=csv,noheader \
...
list_gpu_processes() {
    ...
    # 这里必须走 nvidia_smi_query：驱动坏掉时 nvidia-smi 把失败横幅打到 stdout，
    # 直接取值会把横幅当成"正在占用 GPU 的进程"打出来，还会让下面的 lsof 兜底被跳过。
    pids=$(nvidia_smi_query --query-compute-apps=pid,process_name --format=csv,noheader || true)
...
detect_installed_driver_version() {
    ...
    if [[ -r "$NVIDIA_PROC_VERSION_FILE" ]]; then
        version=$(sed -nE 's/.*Kernel Module +([0-9][0-9.]*).*/\1/p' "$NVIDIA_PROC_VERSION_FILE" 2>/dev/null | head -1)
    fi

    if [[ -z "$version" ]]; then
        version=$(nvidia_smi_query --query-gpu=driver_version --format=csv,noheader | head -1 | tr -d '[:space:]' || true)
    fi

    # 只回传形状合法的版本号：调用方会拿它做 version_ge 比较和文案展示，
    # 放行残缺字符串会让比较失效、日志里出现一行乱码。
    is_version_string "$version" || return 1
...
validate_apt_driver_simulation() {
    local package=$1
    local output=$2
    local kernel_release
    ...
    if grep -qiE '(unmet dependencies|held broken packages)' <<< "$output"; then
        log_error "APT 模拟事务包含依赖错误，拒绝执行。"
        return 1
    fi
    while read -r removed_package; do
        [[ -n "$removed_package" ]] || continue
        if [[ "$removed_package" == *"$kernel_release"* || \
              "$removed_package" == "linux-headers-${kernel_release}" ]]; then
            log_error "APT 模拟事务会移除当前内核或头文件: ${removed_package}"
            return 1
        fi
        case "$removed_package" in
            nvidia-*|libnvidia-*|xserver-xorg-video-nvidia-*|linux-modules-nvidia-*|\
            linux-objects-nvidia-*|linux-signatures-nvidia-*|cuda-drivers*) ;;
            *)
                log_error "APT 模拟事务会移除无关包 ${removed_package}，拒绝执行。"
                return 1
                ;;
        esac
    done < <(awk '$1 == "Remv" { print $2 }' <<< "$output")
    if grep -Eq "^Inst[[:space:]]+${package}([[:space:]]|$)" <<< "$output"; then
        return 0
    fi
    if grep -Fq "${package} is already the newest version" <<< "$output" || package_is_installed "$package"; then
        return 0
    fi

    log_error "APT 模拟事务未包含所选包 ${package}。"
    return 1
}

simulate_apt_driver_transaction() {
    local package=$1
    local output
    ...
    if ! output=$(run_privileged env LC_ALL=C apt-get -s -o Debug::NoLocking=1 install -y -- "$package" 2>&1); then
        log_error "APT Driver 模拟事务失败。"
        print_command_output "$output"
        return 1
    fi
    validate_apt_driver_simulation "$package" "$output" || {
        print_command_output "$output"
        return 1
    }
    APT_DRIVER_SIMULATION="$output"
}
...
detect_nvidia_gpu_count() {
    ...
    command -v nvidia-smi &> /dev/null || return 1
    count=$(timeout --signal=TERM --kill-after=5s "${NVIDIA_PROBE_TIMEOUT_SECONDS:-30}" \
        nvidia-smi --query-gpu=index --format=csv,noheader 2>/dev/null | awk 'NF { count++ } END { print count + 0 }') || return 1
    [[ "$count" =~ ^[0-9]+$ ]] || return 1
    printf '%s\n' "$count"
}

install_selected_driver_apt() {
    local installed_version=""
    local installed_driver_version=""

    ensure_driver_dependencies || return 1
    simulate_apt_driver_transaction "$SELECTED_DRIVER_PACKAGE" || return 1

    print_section "Ubuntu 26.04 NVIDIA Driver APT 事务预检"
    print_kv "目标包" "$SELECTED_DRIVER_PACKAGE"
    print_kv "目标驱动" "$SELECTED_DRIVER_VERSION"
    print_hint "APT 模拟事务已通过：仅允许替换 NVIDIA 驱动栈，无无关包移除或依赖破坏。"

    if [[ "$CUDA_INSTALL_DRY_RUN" == "1" ]]; then
        echo "$APT_DRIVER_SIMULATION"
        log_info "DRY-RUN: no system changes were made"
        return 3
    fi

    save_install_state "${FILENAME:-}" "yes" "${CUDA_VERSION:-}" \
        "$SELECTED_DRIVER_VERSION" "" "" "${RECOMMENDED_DRIVER_VERSION:-}" \
        "apt" "${MINIMUM_DRIVER_VERSION:-}" "${INSTALL_MODE:-driver_only}" || return 1

    PREFLIGHT_GPU_COUNT=$(detect_nvidia_gpu_count || true)
    installed_version=$(installed_apt_package_version "$SELECTED_DRIVER_PACKAGE" || true)
    installed_driver_version=$(normalize_apt_driver_version "$installed_version" || true)
    if [[ "$installed_driver_version" == "$SELECTED_DRIVER_VERSION" ]]; then
        log_info "${SELECTED_DRIVER_PACKAGE} 已是目标版本，跳过 APT 安装。"
        return 0
    fi

    print_section "通过 APT 安装 NVIDIA Driver"
    if ! run_privileged env DEBIAN_FRONTEND=noninteractive apt-get install -y -- "$SELECTED_DRIVER_PACKAGE"; then
        log_error "APT 安装 NVIDIA Driver 失败: ${SELECTED_DRIVER_PACKAGE}"
        return 1
    fi
}

verify_selected_driver_apt() {
    local kernel_release
    local package_version target_package_version disk_version loaded_version smi_version smi_count
    local smi_output

    kernel_release=$(current_kernel_release) || return 1
    package_version=$(installed_apt_package_version "$SELECTED_DRIVER_PACKAGE" || true)
    target_package_version=$(normalize_apt_driver_version "$package_version" || true)
    if [[ "$target_package_version" != "$SELECTED_DRIVER_VERSION" ]]; then
        log_error "已安装包版本不匹配: ${SELECTED_DRIVER_PACKAGE}=${package_version:-未安装}，目标 ${SELECTED_DRIVER_VERSION}。"
        return 1
    fi

    disk_version=$(modinfo -k "$kernel_release" -F version nvidia 2>/dev/null | head -1 || true)
    [[ -n "$disk_version" ]] || disk_version=$(modinfo -F version nvidia 2>/dev/null | head -1 || true)
    if [[ "$disk_version" != "$SELECTED_DRIVER_VERSION" ]]; then
        log_error "当前内核的磁盘 Driver 模块版本不匹配: ${disk_version:-缺失}，目标 ${SELECTED_DRIVER_VERSION}。"
        return 1
    fi

    loaded_version=$(detect_loaded_driver_version || true)
    if [[ -n "$loaded_version" && "$loaded_version" != "$SELECTED_DRIVER_VERSION" ]]; then
        log_warn "磁盘 Driver 已更新为 ${SELECTED_DRIVER_VERSION}，内存仍加载 ${loaded_version}。"
        print_hint "请手动 reboot 后重新运行脚本完成验收。"
        return 2
    fi

    if ! smi_output=$(timeout --signal=TERM --kill-after=5s "${NVIDIA_PROBE_TIMEOUT_SECONDS:-30}" \
        nvidia-smi --query-gpu=driver_version --format=csv,noheader 2>&1); then
        log_error "nvidia-smi 验证失败。"
        print_command_output "$smi_output"
        return 1
    fi
    smi_version=$(head -1 <<< "$smi_output" | tr -d '[:space:]')
    if [[ "$smi_version" != "$SELECTED_DRIVER_VERSION" ]]; then
        log_error "nvidia-smi Driver 版本 ${smi_version:-未知} 与目标 ${SELECTED_DRIVER_VERSION} 不符。"
        return 1
    fi

    if [[ "${PREFLIGHT_GPU_COUNT:-}" =~ ^[1-9][0-9]*$ ]]; then
        smi_count=$(detect_nvidia_gpu_count || true)
        if [[ "$smi_count" != "$PREFLIGHT_GPU_COUNT" ]]; then
            log_error "Driver 安装前后 GPU 数量不一致: ${PREFLIGHT_GPU_COUNT} -> ${smi_count:-0}。"
            return 1
        fi
    fi

    log_success "Ubuntu Driver 包、当前内核模块和 nvidia-smi 验证通过 (${SELECTED_DRIVER_VERSION})。"
}
...
remove_stale_nvidia_ko() {
    ...
    ko_dir="/lib/modules/$(current_kernel_release)/kernel/drivers/video"
...
EOF
    ...
    print_hint "脚本不会自动重启。请手动执行 reboot，重启后再次运行 ./install_cuda.sh。"
    return 2
}
...
if [ -f "$STATE_FILE" ]; then
    ...
    SAVED_DRIVER_BACKEND="${SAVED_DRIVER_BACKEND:-runfile}"
    SELECTED_DRIVER_PACKAGE="${SAVED_DRIVER_PACKAGE:-}"
    ...
    if ! validate_driver_backend "$SAVED_DRIVER_BACKEND"; then
        log_error "状态文件中的 Driver 后端无效: ${SAVED_DRIVER_BACKEND}"
        exit 1
    fi
    if [[ "$CONFIRM_DRIVER" == "yes" && "$SAVED_DRIVER_BACKEND" != "$DRIVER_BACKEND" ]]; then
        log_error "状态文件 Driver 后端 (${SAVED_DRIVER_BACKEND}) 与当前系统 (${DRIVER_BACKEND}) 不一致。"
        print_hint "请删除 ${STATE_FILE} 后重新选择兼容的 Driver。"
        exit 1
    fi
    if [[ "$DRIVER_BACKEND" == "apt" && "$CONFIRM_DRIVER" == "yes" ]]; then
        if ! valid_apt_driver_package "$SELECTED_DRIVER_PACKAGE"; then
            log_error "状态文件中的 Ubuntu Driver 包无效: ${SELECTED_DRIVER_PACKAGE:-空}"
            exit 1
        fi
        if [[ "$SELECTED_DRIVER_SOURCE" != "apt" ]]; then
            log_error "Ubuntu 26.04 状态文件禁止使用 Driver runfile 来源: ${SELECTED_DRIVER_SOURCE}"
            exit 1
        fi
    fi
    ...
    if [[ "$DRIVER_BACKEND" != "apt" ]] && ! validate_nvidia_driver_url "$SELECTED_DRIVER_URL"; then
    ...
    if [[ "$CONFIRM_DRIVER" == "yes" ]]; then
        ...
        [[ -z "$SELECTED_DRIVER_PACKAGE" ]] || print_kv "恢复的 Driver 包" "$SELECTED_DRIVER_PACKAGE"
    fi
    ...
    while true; do
        ...
        if [[ "$INSTALL_MODE" == "driver_only" ]]; then
            ensure_driver_dependencies || exit 1
            if select_driver_only_flow; then
                break
            else
                if [[ -n "$CUDA_INSTALL_MODE" ]]; then
                    log_error "非交互 Driver-only 选择失败，已退出。"
                    return 1
                fi
                continue
            fi
    ...
    while true; do
        ...
        if [[ "$DRIVER_BACKEND" == "apt" ]]; then
            ensure_driver_dependencies || exit 1
            APT_SELECTION_STATUS=0
            select_apt_driver_for_cuda || APT_SELECTION_STATUS=$?
            case "$APT_SELECTION_STATUS" in
                0) ;;
                10) continue ;;
                *) exit 1 ;;
            esac
...
if [[ "$INSTALL_MODE" != "driver_only" ]]; then
    ...
    if [[ "$CUDA_INSTALL_DRY_RUN" != "1" ]]; then
        ...
        print_kv "DRY-RUN Toolkit 安装器" "$DOWNLOAD_URL"
    fi
fi

if [[ "$CUDA_INSTALL_DRY_RUN" == "1" && \
      ( "$CONFIRM_DRIVER" != "yes" || "$DRIVER_BACKEND" != "apt" ) ]]; then
    log_info "DRY-RUN: no system changes were made"
    return 0
fi
...
if [[ "$CONFIRM_DRIVER" == "yes" ]]; then
    ...
    if [[ "$DRIVER_BACKEND" == "apt" ]]; then
        ...
        install_selected_driver_apt || APT_INSTALL_STATUS=$?
        case "$APT_INSTALL_STATUS" in
            0) ;;
            3) return 0 ;;
            *) return 1 ;;
        esac
...
if [[ "$INSTALL_MODE" == "driver_only" ]]; then
    ...
    if [[ "$DRIVER_BACKEND" == "apt" ]]; then
        APT_VERIFY_STATUS=0
        verify_selected_driver_apt || APT_VERIFY_STATUS=$?
        case "$APT_VERIFY_STATUS" in
            0) ;;
            2)
                log_warn "NVIDIA Driver 包已安装，但需要手动重启后才能完成验收。"
                return 2
                ;;
            *)
                log_error "Ubuntu NVIDIA Driver 安装后验收失败，状态文件已保留。"
                return 1
                ;;
        esac
    else
...
if [[ "$DRIVER_BACKEND" == "apt" && "${DRIVER_NEEDS_REBOOT:-0}" == "1" ]]; then
    log_warn "Toolkit 已安装，但 NVIDIA Driver 仍需手动重启后完成验收。"
    print_hint "状态文件已保留；reboot 后重新运行脚本。"
    return 2
fi
...
if [[ "${INSTALL_CUDA_SOURCE_ONLY:-0}" != "1" ]]; then
    main "$@"
fi
