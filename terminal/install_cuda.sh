    ...
    if command -v systemctl &> /dev/null; then
        ...
        for service in nvidia-fabricmanager nvidia-fabric-manager nvidia-persistenced \
            ...
            if systemctl is-active --quiet "$service" 2>/dev/null; then
                ...
                # 仅 stop 不够：安装器会执行 systemctl daemon-reload，
                # enabled 的服务可能被重新拉起并 modprobe 旧模块，
                # 造成 nvidia.ko(旧) 与 nvidia-modeset.ko(新) 版本冲突。
                # mask 在 /etc/systemd/system 建符号链接，若该目录已有同名
                # 单元文件会失败，此时退回 disable。
                if run_privileged systemctl mask "$service" 2>/dev/null; then
                    MASKED_NVIDIA_SERVICES+=("$service")
                    log_info "已临时屏蔽 ${service}，防止安装期间自动重启。"
                    ...
                    print_hint "如安装后出现 Driver/library version mismatch，重启即可恢复。"
                fi
            fi
        done
    fi
...
append_driver_kernel_module_type() {
    ...
    [[ -z "$NVIDIA_KERNEL_MODULE_TYPE" ]] && return 0
    ...
    if [[ "$help_text" == *"--kernel-module-type"* ]]; then
        module_arg="--kernel-module-type=${NVIDIA_KERNEL_MODULE_TYPE}"
    elif [[ "$help_text" == *"-m=kernel-open"* ]]; then
        if [[ "$NVIDIA_KERNEL_MODULE_TYPE" == "open" ]]; then
            module_arg="-m=kernel-open"
        else
            module_arg="-m=kernel"
        fi
    else
        log_warn "当前驱动 runfile 未显示内核模块类型选项，忽略 NVIDIA_KERNEL_MODULE_TYPE 设置（由安装器自行选择）。"
        return 0
    fi

    DRIVER_INSTALL_ARGS+=("$module_arg")
    log_info "内核模块类型: ${NVIDIA_KERNEL_MODULE_TYPE}（参数 ${module_arg}）。"
}
