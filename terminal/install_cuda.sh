...
version_ge() {
    ...
    # 段数对齐(缺失段补 0)，避免 sort -V 把 12.4 误判为小于 12.4.0
    IFS=. read -ra c_parts <<< "$candidate"
    IFS=. read -ra m_parts <<< "$minimum"
    while (( ${#c_parts[@]} < ${#m_parts[@]} )); do c_parts+=(0); done
    while (( ${#m_parts[@]} < ${#c_parts[@]} )); do m_parts+=(0); done
    c_norm=$(IFS=.; printf "%s" "${c_parts[*]}")
    m_norm=$(IFS=.; printf "%s" "${m_parts[*]}")

    first=$(printf "%s\n%s\n" "$m_norm" "$c_norm" | sort -V | head -n1)
    [[ "$first" == "$m_norm" ]]
}
...
get_release_notes() {
    ...
    if wget -qO "$temp_file" "$release_notes_url" 2>/dev/null && release_notes_html_is_valid "$temp_file"; then
